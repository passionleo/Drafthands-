...
export const SubscriptionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Reactive state for owner-permitted visitor access and site access policy
  const [isPermittedByOwner, setIsPermittedByOwner] = useState<boolean>(() => {
    return VisitorTracker.isCurrentVisitorPermitted();
  });

  useEffect(() => {
    const handlePermitChange = () => {
      setIsPermittedByOwner(VisitorTracker.isCurrentVisitorPermitted());
    };
    window.addEventListener('drafthands_visitor_permitted', handlePermitChange);
    window.addEventListener('drafthands_visitor_revoked', handlePermitChange);
    window.addEventListener('drafthands_policy_changed', handlePermitChange);
    return () => {
      window.removeEventListener('drafthands_visitor_permitted', handlePermitChange);
      window.removeEventListener('drafthands_visitor_revoked', handlePermitChange);
      window.removeEventListener('drafthands_policy_changed', handlePermitChange);
    };
  }, []);
  ...
  // isSubscribed reflects strict active paid status, master bypass, or owner permit
  const isSubscribed = Boolean(isMasterAdmin || hasActivePaidSubscription || isPermittedByOwner);
  ...
  const checkTopicAccess = (topic: DrawingTopic, topicsInTier: DrawingTopic[]) => {
    // If master admin, active paid subscription, or permitted by owner, all syllabus modules are fully unlocked
    if (isMasterAdmin || hasActivePaidSubscription || isPermittedByOwner) {
  ...
  const redeemVoucherCode = (rawCode: string) => {
    ...
    // VIP Access Pass check generated by Owner
    if (code.startsWith('VIP-') || code.startsWith('PASS-')) {
      const vipResult = VisitorTracker.redeemVipPass(code);
      if (vipResult.success) {
        setIsPermittedByOwner(true);
        return {
          success: true,
  ...
  const openPaywall = (topic?: DrawingTopic) => {
    if (isMasterAdmin || isPermittedByOwner) {
      console.info('[DraftHands] Master Owner or Permitted Access active. Paywall modal blocked.');
      return;
    }
    if (topic) {
